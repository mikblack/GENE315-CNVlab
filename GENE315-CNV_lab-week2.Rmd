...
* Continuing looking at the count data from last week, focusing on the two loci (FCGR, IRGM) across the three populations (CEU, CHB, YRI).
* Looking at single nucleotide variant (SNV) data from SNVs near the IRGM and FCGR3B genes.
* Combining the SNV and CNV data to investigate linkage disequilibrium (LD) in the three populations.

This document will walk through some simple analysis in R to accomplish the tasks above. A copy is available online at:

https://github.com/mikblack/GENE315-CNVlab/blob/master/GENE315-CNV_lab-week2.md

__REMINDER: FOR YOUR ASSIGNMENT YOU WILL NEED TO ALTER THE CODE BELOW TO PERFORM A SIMILAR ANALYSIS FOR THE IRGM DATA AND THE RELEVANT SNV (rs13361189).__
...
These data are saved in the file ```CNcalls.csv``` in the ```DataFiles``` folder.  You can load them into R via:
...
The file ```CNcalls.csv``` contains genotype data for SNVs near the IRGM and FCGR3B genes.  For IRGM the SNV ID is ```rs13361189```, and for FCGR3B the SNV ID is ```rs117435514```.  These data are stored in the files ```IRGM_rs13361189.csv``` and ```FCGR_rs117435514.csv``` (in the ```DataFiles``` folder).

These data were obtained from the ensembl website.  Have a look at the 1000 Geneomes Project data for SNV ```rs117435514``` by searching for it at

https://www.ensembl.org/Homo_sapiens/Info/Index

and then clicking on the "population genetics" link.  This provides data on SNV frequency (and also per-sample genotype) across the populations.  The data from this website was used to create SNV genotype files for the samples we are analysing in this lab.
...
which shows that most individuals have the AA genotype, and relatively few have the AG genotype (and none have the GG genotype).  Also, note that some of the samples have missing genotype data for this SNV, denoted by `<NA>` in the output above.

These genotypes can also be viewed across population groups, which shows that most of the genotypic variation in this SNV occurs in the CHB population:
...
If a specific SNV genotype is a associated with a change in copy number in a particular region, then that SNV can be said to *tag* that copy number variant.  That is, an individual with that SNV genotype is more likely to also have altered copy number for that region.

We can investigate this at the FCGR locus by combining the copy number calls for FCGR3B with the genotype data for SNV rs117435514 (note that this works because I've made sure that all of the data are in the same order in each file):
...
So, even though the SNV rs117435514 does not tag CNV at FCGR3B particularly well, the association is highly statistically significant (i.e., it is very unlikely that we would see a relationship this strong by chance).

## Assignment

The assignment for this module is due at **noon** on 22 April (Wednesday stream) or 23 April (Thursday stream).  For your document, please provide answers to the questions below, and also the questions at the end of the week 2 and week 3 handouts.  **When answering each question, please provide the R code used to generate the output (if required), the output itself, and any comments/discussion needed to fully answer the question.**  Please keep the code, output and comments together for each question (similar to how the lab handouts are laid out).
...
3.	Load the rs13361189 SNV data, and generate a table showing the genotype frequencies.  What is the most frequently observed geneotype?

4.	Look at the frequency of rs13361189 across the three populations - generate a table, a proportion table, and a barplot for these calls across the three populations. Formally test for an association between population and rs13361189 SNV genotype.  Comment on your results, and provide an interpretation in terms of rs13361189 genotype frequencies in these populations.

5.	Combine the rs13361189 SNV genotypes and copy number data relating to IRGM. Create a table of genotype versus copy number, and peform a formal test for association.  Provide an interpretation of your results. How good is rs13361189 genotype at tagging loss of the region upstream of IRGM?

6.  Do your results (in terms of association between rs13361189 genotype and IRGM copy number) agree with those presented by McCarroll et al. (2008)? Provides detals to justify your conclusion. What role is the IRGM deletion thought to play in increasing Crohn's Disease risk?

    McCarroll, S., Huett, A., Kuballa, P. et al. Deletion polymorphism upstream of IRGM associated with altered IRGM expression and Crohn's disease. *Nature Genetics* 40, 1107–1112 (2008).  https://doi.org/10.1038/ng.215
