---
title: "Appendix 1: R code - week 1"
author: "Mik Black"
date: "6 & 7 April 2016"
...
In this part of the lab you will be looking at copy number variation in samples 
from the 1000 Genomes Project, using publicly available high-throughput sequencing 
data.  The link for the 1000 Genomes Project is:

http://1000genomes.org

The first couple of questions for your lab report relate to the 1000 Genomes 
Project - you should be able to find the answers to those questions by reading 
the website.

Aligned sequence data from the 1000 Genomes Project was obtained from their
ftp download site:

ftp://ftp.1000genomes.ebi.ac.uk/vol1/ftp/

For this lab we are using a relatively small subset of the full data (310 samples)
out of 2535, from three of the 26 populations included in the study.

As mentioned in class, the aligned data for each sample is stored in the BAM format
on the ftp server.  One of the nice things about BAM is that you can *index* the
data files, which makes it possible to retrieve a subset of the data (e.g., from
a particular region of the genome) without having to download the entire data file.

This is an important point, as the data files for a single sample are fairly large 
(e.g., around 30GB).  For this lab I have downloaded data from regions around the three genes of 
interest (AMY1A, IRGM and FCGR3B).  For each of the three regions, the data extracted
...
To do this we can use the _samtools_ application (or the _Rsamtools_ package from within R).  Unfortunately (or luckily, depending on how much you like writing code) the Rsamtools was slightly too tricky to install on the Student Desktop, so I have carried out this process for you.

## Getting started in R

We will be performing our R-based analysis using the Rstudio application on the Student Desktop. To begin using Rstudio, log in to the Student Desktop:

https://student.desktop.otago.ac.nz/

and find Rstudio in the Windows start menu (bottom left of the screen).

## Downloading the data

The data and R files needed for this lab are located on Blackboard in a zip file
in the "CNV Computer Lab - week 1" folder.  Download this file to the desktop, and unzip it.  In R, set your working directory to be the folder that is created when the file is unzipped.
...
The analyses below relate to the data from the FCGR locus.  For your lab report, you need to 
...
We can use this to automate data processing - the following exaple calculates the
...
column ranges are lsited in the table in the "Data generation parameters and samples"
